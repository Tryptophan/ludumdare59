...
func setup(pos: Vector2, size: Vector2) -> void:
	...
	for i in range(1, ITEM_COUNT + 1):
		var tex = load("res://assets/case-opening/%d.png" % i) as Texture2D
		if tex != null:
			_textures[i] = tex
...
func _show_case() -> void:
	_phase = "idle"
	var case_tex = load("res://assets/case-opening/case.png") as Texture2D
...
func _start_spin() -> void:
	...
	spin_audio.stream = load("res://assets/case-opening/spin.wav")
	...
	# calculate starting scroll so strip starts from left and landing item ends at center
	var landing_index = STRIP_ITEMS - 10
	...
	# scroll_offset represents how much we have scrolled so far
	_scroll_offset = 0.0
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		if _case_sprite and _case_sprite.get_rect().grow(20).has_point(_case_sprite.to_local(to_global(local_pos))):
			_start_spin()
